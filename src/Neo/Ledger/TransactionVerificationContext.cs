...
namespace Neo.Ledger
{
    ...
    public class TransactionVerificationContext
    {
        ...
        public bool CheckTransaction(Transaction tx, IEnumerable<Transaction> conflictingTxs, DataCache snapshot)
        {
            ...
            BigInteger balance;
            if (isSponsored)
                balance = NativeContract.Notary.BalanceOf(snapshot, payer.Item2);
            else
                balance = NativeContract.GAS.BalanceOf(snapshot, payer.Item1);
            ...
            foreach (var conflictTx in conflictingTxs.Where(c =>
            {
                ...
                return conflictPayer.Item1 == payer.Item1 && conflictPayer.Item2 == payer.Item2;
            }))
