...
public class UT_NativeContract
{
    ...
    {
        Assert.AreEqual(NativeContract.GetContract(NativeContract.Governance.Hash), NativeContract.Governance);
    }

    [TestMethod]
    public void TestGenesisNeoGasTokenState()
    {
        ...
        Assert.IsNotNull(neoTokenState, "NeoToken TokenState should exist in genesis block");
        Assert.AreEqual(Governance.NeoTokenName, neoTokenState.Name, "NeoToken name should match");
        Assert.AreEqual(Governance.NeoTokenSymbol, neoTokenState.Symbol, "NeoToken symbol should match");
        Assert.AreEqual(Governance.NeoTokenDecimals, neoTokenState.Decimals, "NeoToken decimals should match");
        Assert.AreEqual(TokenType.Fungible, neoTokenState.Type, "NeoToken should be fungible");
        Assert.AreEqual(NativeContract.Governance.Hash, neoTokenState.Owner, "NeoToken owner should be Governance contract");

        // Check GasToken TokenState exists
        var gasTokenState = NativeContract.TokenManagement.GetTokenInfo(snapshot, gasTokenId);
        Assert.IsNotNull(gasTokenState, "GasToken TokenState should exist in genesis block");
        Assert.AreEqual(Governance.GasTokenName, gasTokenState.Name, "GasToken name should match");
        Assert.AreEqual(Governance.GasTokenSymbol, gasTokenState.Symbol, "GasToken symbol should match");
        Assert.AreEqual(Governance.GasTokenDecimals, gasTokenState.Decimals, "GasToken decimals should match");
        Assert.AreEqual(TokenType.Fungible, gasTokenState.Type, "GasToken should be fungible");
        Assert.AreEqual(NativeContract.Governance.Hash, gasTokenState.Owner, "GasToken owner should be Governance contract");
    }
    ...
    public void TestNativeContractId()
    {
        ...
        Assert.AreEqual(-14, NativeContract.TemporaryStorage.Id);
    ...
    public void TestGenesisNativeState()
    {
        ...
        // Note: NeoToken is no longer a native contract, it's managed by Governance through TokenManagement
